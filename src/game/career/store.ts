...
function withInternationalForm(
  ...
  if (!club) return sim;
  const publicSeason = internationalCalendarSeason(ctx?.seasonNumber ?? 1, {
    leagueOnly: ctx?.role === 'reserve',
    careerStart: ctx?.careerStart,
    role: ctx?.role,
  });
...
function openNextSimFixture(state: CareerState): Partial<CareerState> {
  ...
  const sitOutHub = (): Partial<CareerState> => {
    ...
    return {
      ...
      seasonCalendar: calendar,
      currentSeason: reviewed.currentSeason,
      squadStatus: reviewed.squadStatus,
      availability,
      nationalTeam,
      careerGames,
  ...
  while (sim.fixtureIndex < calendar.fixtures.length) {
    ...
    const chances = chancesForSquadStatus(
      state.squadStatus ?? defaultSquadStatus(state.role),
...
function finishResolvedLiveMatch(
  ...
  const selectedSim = withInternationalForm(
    ...
    nextCareerGames,
    {
      week: currentCalendarWeek(nextCalendar, nextSim.fixtureIndex),
      seasonNumber: state.seasonNumber,
      role: state.role,
      careerStart: state.careerStart,
  ...
  const reviewed = complete
    ? { squadStatus: state.squadStatus, currentSeason: awarded.season }
    : reviewedSquadFields(
        ...
        set((state) => {
          ...
          const nextStatus = transition.immediate?.squadStatus
            ...
            ?? nextSquadStatusAfterSeason({
              ...
              bar: threshold,
              honoursClear: seasonOverridesRatioBar(season),
              allowRisingStar: publicSeason === 1,
            });
          const finishedSeason: SeasonRecord = {
            ...season,
            ratioMet: seasonRatioClearsBar({
              ratio,
              gamesPlayed: season.gamesPlayed,
              bar: threshold,
              season,
        ...
        set((state) => {
          ...
          if (clubId === null) {
            ...
            const stayStatus = stay.squadStatus
              ?? nextSquadStatusAfterSeason({
                ...
                honoursClear: state.currentSeason ? seasonOverridesRatioBar(state.currentSeason) : false,
              });
          ...
          if (offer && !offer.renewal && offer.clubId !== state.clubId) {
            ...
            const playerValue = valueClub
              ...
              : 0;
            const remainingPermanentOffers = (pending.offers ?? []).filter(
              (item) => item !== offer && item.move === 'permanent' && !item.renewal,
            ).length;
          ...
          const loanWage = offer?.weeklyWage && offer.weeklyWage > 0
            ? offer.weeklyWage
            : state.weeklyWage;
          ...
          const nextIfStay = pending.stay?.squadStatus
            ?? nextSquadStatusAfterSeason({
              ...
              honoursClear: state.currentSeason ? seasonOverridesRatioBar(state.currentSeason) : false,
              allowRisingStar: displaySeasonNumber(state.seasonNumber, {
                role: state.role,
                careerStart: state.careerStart,
              }) === 1,
            });
        ...
        return {
          ...
          squadStatus: (() => {
            ...
            if (
              state.role === 'first-team'
              && status === 'starter'
              && displaySeasonNumber(state.seasonNumber ?? 1, {
                role: state.role,
                careerStart: state.careerStart,
              }) === 1
            ) {
            ...
            ? {
                ...
                stay: state.pendingTransfer.stay
                  ? {
                      ...
                      squadStatus: state.pendingTransfer.stay.squadStatus
                        ? normalizeSquadStatus(state.pendingTransfer.stay.squadStatus)
                        : state.pendingTransfer.stay.squadStatus,
                    }
                  : state.pendingTransfer.stay,
